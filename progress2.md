...
**增益（Gain）**：文档中两种口径并存，引用时注明：
- **§45（sec50）口径**：$\mathrm{Gain}=\dfrac{A-C}{A}$（正 = 预训练 C 优于 scratch A；基准 = A）
- **§49.13/§50（e500/ft150）口径**：$\mathrm{Gain}=\dfrac{A-C}{C}$（基准 = C）
- 同一数字两种口径不同（如 sec50 5% 档：基准 A 得 +50.5%，基准 C 得 +102%），
  跨表对比前必须统一
...
> 指标 = nrmse_per_sample %（3 seed 均值±std）；增益 = (A−C)/A（temporal）或 (A−C)/C（random）。
...
|---|---|---|---|---|---|---|---|---|
| 5% | 1.107 ± 0.022 | **0.974 ± 0.012** | **+13.7%** | 0.981 ± 0.022 | +12.8% | 0.991 ± 0.011 | +11.7% | `pca_w150_*_5pct_*.json` |
| 1% | 1.483 ± 0.039 | **1.076 ± 0.010** | **+37.8%** | 1.057 ± 0.016 | **+40.2%** | 1.147 ± 0.038 | **+29.2%** | |
...
|---|---|---|---|---|---|
| Random split | 5% | 1.107 | **0.974 ± 0.012** | **+13.7%** | §50.12（w150，clean）|
| Random split | 1% | 1.483 | **1.076 ± 0.010** | **+37.8%** | §50.12（w150，clean；A = {s54,s56,s58}，§57.3）|
...
> Gain = (A−C)/C（全精度计算后 1 位小数）。全表仅 1% 行随 §57 更新（旧 1.464/+36.1/+38.5/+27.6），
...
|---|---|---|---|---|---|---|---|
| 1% | 1.483 ± 0.039 | 1.076 ± 0.010 | **+37.8%** | 1.057 ± 0.016 | **+40.2%** | 1.147 ± 0.038 | **+29.2%** |
| 5% | 1.107 ± 0.022 | 0.974 ± 0.012 | **+13.7%** | 0.981 ± 0.022 | **+12.8%** | 0.991 ± 0.011 | **+11.7%** |
| 25% | 0.772 ± 0.039 | 0.815 ± 0.021 | −5.3% | 0.809 ± 0.040 | −4.6% | 0.807 ± 0.024 | −4.4% |
| 50% | 0.714 ± 0.013 | 0.729 ± 0.013 | −2.1% | 0.764 ± 0.037 | −6.6% | 0.749 ± 0.030 | −4.7% |
| 100% | 0.659 ± 0.025 | 0.686 ± 0.032 | −4.0% | 0.658 ± 0.011 | +0.1% | 0.653 ± 0.003 | +0.9% |
...
noisy05 = 1.0572 ± 0.0161、noisy20 = 1.1474 ± 0.0377；Gain = 37.85 / 40.24 / 29.22%→ 报 +37.8/+40.2/+29.2%。
...
（nrmse_per_sample %；表源 = §59.2 random（w150 clean）/ §59.3 temporal（u1e4 clean）。⚠️ 论文表原报口径不同：
random = (A−C)/C（§59.2：1% +37.8% 等），temporal = (A−C)/A（§59.3：+11.4% 等）；本表统一换算为 (A−C)/A。）
...
| Gain 公式 | **(A−C)/C**（基准 = C）| **(A−C)/A**（基准 = A）⚠️ 两表基准不同 |
...
\midrule
1\%   & $1.483 \pm 0.039$ & $1.076 \pm 0.010$ & $\bm{+37.8\%}$ & $1.057 \pm 0.016$ & $\bm{+40.2\%}$ & $1.147 \pm 0.038$ & $\bm{+29.2\%}$ \\
5\%   & $1.107 \pm 0.022$ & $0.974 \pm 0.012$ & $\bm{+13.7\%}$ & $0.981 \pm 0.022$ & $\bm{+12.8\%}$ & $0.991 \pm 0.011$ & $\bm{+11.7\%}$ \\
25\%  & $0.772 \pm 0.039$ & $0.815 \pm 0.021$ & $-5.3\%$ & $0.809 \pm 0.040$ & $-4.6\%$ & $0.807 \pm 0.024$ & $-4.4\%$ \\
50\%  & $0.714 \pm 0.013$ & $0.729 \pm 0.013$ & $-2.1\%$ & $0.764 \pm 0.037$ & $-6.6\%$ & $0.749 \pm 0.030$ & $-4.7\%$ \\
100\% & $0.659 \pm 0.025$ & $0.686 \pm 0.032$ & $-4.0\%$ & $0.658 \pm 0.011$ & $+0.1\%$ & $0.653 \pm 0.003$ & $+0.9\%$ \\
\bottomrule
\end{tabular}
...
- 口径警示（跨表引用必读，附录 A）：① random Gain=(A−C)/C，temporal Gain=(A−C)/A；
