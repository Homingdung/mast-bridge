...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Global multiplier applied to every sigma in --noise-profile "
        "(e.g. 4.0 = 4x measurement noise).",
    )
...
def _strictly_accepted(row: dict[str, Any]) -> bool:
    try:
        ...
        return False
    # NOTE: topology is not filtered here.  The accepted manifest already
    # anchored topology to EFIT at target_time (criterion 2); limited solves
    # are legitimate accepted samples and must receive diagnostics too.
    if row.get("solver_converged") is not True or tolerance > STRICT_SOLVER_TOLERANCE:
        return False
    from mast_bridge.dataset.synthetic_quality import solver_instrumented_rejection_reasons

    return not solver_instrumented_rejection_reasons(row)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.noise_profile is not None:
        ...
        if args.noise_scale != 1.0:
            noise_profile = {
                name: float(sigma) * args.noise_scale for name, sigma in noise_profile.items()
            }
        print(
            f"noise_profile: {len(noise_profile)} features, seed={args.noise_seed}, "
            f"scale={args.noise_scale}"
        )
